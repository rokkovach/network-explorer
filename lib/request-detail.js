...
const RequestDetail = {
  ...
  init() {
    ...
    document.getElementById('btn-copy-curl').addEventListener('click', () => {
      ...
      Storage.copyToClipboard(Storage.generateCurl(this.currentRequest, false));
      showToast('cURL copied to clipboard');
    });
    document.getElementById('btn-copy-curl-masked').addEventListener('click', () => {
      if (!this.currentRequest) return;
      Storage.copyToClipboard(Storage.generateCurl(this.currentRequest, true));
      showToast('Masked cURL copied to clipboard');
    });
