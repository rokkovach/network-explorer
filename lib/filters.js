  ...
  _renderRules() {
    ...
    this._rules.forEach((rule, idx) => {
      ...
      if (idx > 0) {
        ...
        logicSel.addEventListener('change', () => { this._rules[idx].logic = logicSel.value; });
      ...
      if (rule.field === 'custom') {
        ...
        ci.addEventListener('input', () => { this._rules[idx].customHeader = ci.value; });
        div.appendChild(ci);
      }
      fieldSel.addEventListener('change', () => { this._rules[idx].field = fieldSel.value; this._renderRules(); });
      // Operator
      ...
      opSel.addEventListener('change', () => { this._rules[idx].operator = opSel.value; });
      div.appendChild(opSel);
      // Value input
      const valInput = document.createElement('input');
      valInput.type = 'text'; valInput.placeholder = 'pattern...'; valInput.value = rule.value || '';
      valInput.addEventListener('input', () => { this._rules[idx].value = valInput.value; RequestList.render(); });
      ...
      methodSel.addEventListener('change', () => { this._rules[idx].method = methodSel.value; RequestList.render(); });
      div.appendChild(methodSel);
      // Remove
      const removeBtn = document.createElement('button');
      removeBtn.className = 'remove-rule'; removeBtn.innerHTML = '&times;';
      removeBtn.addEventListener('click', () => { this._rules.splice(idx, 1); this._renderRules(); RequestList.render(); });
